...
with pd.ExcelWriter(r'D:\TemporaryDirectory\NOISY\NOISY.SVR.xlsx') as writer:
...
param_grid = {
    ...
    'C':  np.arange(1,10,1),
    'epsilon':np.arange(0.001    ,0.1,0.001) ,  # 添加 epsilon
}
...
# 创建网格搜索对象，并指定交叉验证策略
grid_search = GridSearchCV(estimator=svr, param_grid=param_grid, scoring=scoring, cv=10, verbose=2)

# 在训练数据上执行网格搜索（包括超参数搜索和交叉验证）
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\NOISY\Predictions_SVR.xlsx') as writer:

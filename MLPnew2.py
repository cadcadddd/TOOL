...
def randomGeneration(NumberOfRows,m):
    ...
    for _ in range(NumberOfRows):
        gene = [
            random.randint(1, m),  # hidden_layer_sizes1
            random.randint(1, m),# hidden_layer_sizes2
            random.randint(1, m),#max_iter
            random.uniform(1,m),  # learning_rate_init
            random.uniform(1,m)#alpha
        ]
...
def fitness(population_list):
    ...
    for individual in population_list:
        try:
            model =MLPRegressor(
                hidden_layer_sizes=(int(individual[0] ),int(individual[0] ) ),
                max_iter=int(individual[2])*300,
                learning_rate_init=individual[3]/20,
                alpha=individual[4]/200,
                activation='tanh',
                solver='sgd',
                random_state = 999,
            )
...
for epoch in range(epochs):
    ...
    if valid_results:
        ...
        if best_params is None or valid_results[0]['rmse'] < best_params['rmse']:
            ...
            print(
                f"Best parameters after epoch {epoch + 1}: hidden_layer_sizes1={current_best[0]}, hidden_layer_sizes2={current_best[1]},max_itermax_iter={current_best[2]}, learning_rate_init={current_best[3]}, alpha={current_best[4]}")
...
final_model=MLPRegressor(
        hidden_layer_sizes=(best_params['params'][0],best_params['params'][1]),
        max_iter=best_params['params'][2]*200,
        learning_rate_init=best_params['params'][3]/200,
        alpha=best_params['params'][4]/100,
        activation='tanh',
        solver='sgd',
        random_state=999,
    )
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\MLP-DH2.xlsx') as writer:

...
def cross_over(generation_list, p):
    if p % 2 != 0:
        p -= 1  # 如果是奇数，减少一个个体
...
for epoch in range(epochs):
    ...
    if valid_results:
        ...
        if best_params is None or valid_results[0]['rmse'] < best_params['rmse']:
            ...
            print(
                f"Best parameters after epoch {epoch + 1}: C={current_best[0]}, epsilon={current_best[1]}")
...
if best_params:
    ...
    # 使用最佳参数训练最终模型
    final_model =SVR(
        C=best_params['params'][0] ,
        epsilon=best_params['params'][1]/1000 ,
        kernel='rbf'
    )
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\SVR-DH.xlsx') as writer:

...
def randomGeneration(NumberOfRows,m):
    ...
    for _ in range(NumberOfRows):
        gene = [
            random.randint(1, m),  # hidden_layer_sizes
    ...
    for individual in population_list:
        try:
            ...
            for train_index, test_index in kf.split(X):
                ...
                model =MLPRegressor(
                    hidden_layer_sizes=int(individual[0] ),
                    max_iter=int(individual[1])*300,
                    learning_rate_init=individual[2]/20,
                    alpha=individual[3]/200,
                    activation='tanh',
                    solver='sgd',
                    random_state = 999,
                 )
...
for epoch in range(epochs):
    ...
    if valid_results:
        ...
        if best_params is None or valid_results[0]['rmse'] < best_params['rmse']:
            ...
            print(
                f"Best parameters after epoch {epoch + 1}: hidden_layer_sizes={current_best[0]}, max_itermax_iter={current_best[1]}, learning_rate_init={current_best[2]}, alpha={current_best[3]}")
...
if best_params:
    ...
    final_model=MLPRegressor(
        hidden_layer_sizes=best_params['params'][0],
        max_iter=best_params['params'][1]*30,
        learning_rate_init=best_params['params'][2]/20,
        alpha=best_params['params'][3]/200,
        activation='tanh',
        solver='sgd',
        random_state=999,
    )
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\MLP-DH.xlsx') as writer:

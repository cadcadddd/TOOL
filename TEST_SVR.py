...
# 使用最佳模型进行预测
y_test_pred_scaled = SVR_model.predict(X_test_scaled)  # 确保使用带有特征名称的 DataFrame
y_train_pred_scaled = SVR_model.predict(X_train_scaled)
Y_BT_pred_scaled = SVR_model.predict(X_BT_scaled)
...
joblib.dump(SVR_model, "SVR_model.pkl")  # 推荐（压缩格式）
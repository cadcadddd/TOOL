...
n = 4  # Number of parameters (like hyperparameters)
...
def randomGeneration(NumberOfRows, NumberOfQueens, m):
    ...
    for i in range(NumberOfRows):
        gene = [
            ...
            random.randint(1, m)#Maximum features
        ]
...
for epoch in range(epochs):
    ...
    if valid_results:
        ...
        if best_params is None or valid_results[0]['rmse'] < best_params['rmse']:
            ...
            print(
                f"Best parameters after epoch {epoch + 1}: n_estimators={current_best[0]}, learning_rate={current_best[1]}, max_depth={current_best[2]},max_featrue={current_best[3]}")
...
if best_params:
    ...
    final_model = AdaBoostRegressor(
        base_estimator=DecisionTreeRegressor(max_depth=current_best[2] + 1,max_features = current_best[3]/10  ),
        n_estimators=current_best[0] * 30,  # 使用 n_estimators
        learning_rate=current_best[1] / 10,  # 使用 learning_rate
    )
...
# 保存为 .pkl
joblib.dump(final_model, "AdaBoost_model.pkl")  # 推荐（压缩格式）
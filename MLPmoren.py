...
# 创建MLPRegressor对象
model = MLPRegressor(random_state=999)
...
# 使用最佳模型进行预测
y_test_pred_scaled =model.predict(X_test_scaled).ravel()
y_train_pred_scaled=model.predict(X_train_scaled).ravel()
Y_BT_pred_scaled   =model.predict(X_BT_scaled).ravel()
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\MLP-MR.xlsx') as writer:

...
def evaluate(params):
    ...
    colsample_bytree = constrain(params[4], 0.01, 1.0)
    colsample_bylevel = constrain(params[5], 0.01, 1.0)
    colsample_bynode = constrain(params[6], 0.01, 1.0)
...
def custom_mutate(individual):
    for i in range(len(individual)):
        if random.random() < 0.7:  # 70%的概率进行变异
            if i == 0:  # learning_rate
                individual[i] = random.uniform(0.01, 1)
            elif i == 1:  # n_estimators
                individual[i] = random.randint(300, 1500)  # 保证是整数
            elif i == 2:  # max_depth
                individual[i] = random.randint(1, 15)  # 保证是整数
            elif i == 3:  # subsample
                individual[i] = random.uniform(0.01, 1.0)
            elif i == 4:  # colsample_bytree
                individual[i] = random.uniform(0.01, 1.0)
            elif i == 5:  # colsample_bylevel
                individual[i] = random.uniform(0.01, 1.0)
            elif i == 6:  # colsample_bynode
                individual[i] = random.uniform(0.01, 1.0)
    return individual,
...
toolbox.register("colsample_bytree", random.uniform, 0.01, 1.0)
toolbox.register("colsample_bylevel", random.uniform, 0.01, 1.0)
toolbox.register("colsample_bynode", random.uniform, 0.01, 1.0)

# 创建个体和种群
toolbox.register("individual", tools.initCycle, creator.Individual,
                 (toolbox.learning_rate, toolbox.n_estimators, toolbox.max_depth,
                  toolbox.subsample, toolbox.colsample_bytree,
                  toolbox.colsample_bylevel, toolbox.colsample_bynode), n=1)
...
def main():
    ...
    for gen in range(200):  # 迭代10代
        # 评估所有个体的适应度
        fitnesses = list(map(toolbox.evaluate, pop))
        for ind, fit in zip(pop, fitnesses):
            ind.fitness.values = fit
        # 打印当前代中每个个体的 n_estimators 和 max_depth
        print(f"\nGeneration {gen + 1}:")
        for ind in pop:
            n_estimators = int(ind[1])  # n_estimators
            max_depth = int(ind[2])      # max_depth
            print(f"Individual: n_estimators={n_estimators}, max_depth={max_depth}")
        # 选择
...
best_model = xgb.XGBRegressor(
    learning_rate=constrain(best_params[0], 0.01, 0.1),
    n_estimators=int(best_params[1]),
    max_depth=int(best_params[2]),
    subsample=constrain(best_params[3], 0.01, 1.0),
    colsample_bytree=constrain(best_params[4], 0.01, 1.0),
    colsample_bylevel=constrain(best_params[5], 0.01, 1.0),
    colsample_bynode=constrain(best_params[6], 0.01, 1.0),
    random_state=999
)
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\XGBoost-DH.xlsx') as writer:

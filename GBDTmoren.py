...
# 定义梯度提升决策树模型
gbdt = GradientBoostingRegressor(random_state=999)
gbdt.fit(X_train, y_train)

# 查看当前超参数设置
print("当前超参数组合：", gbdt.get_params())

# 进行预测
y_test_pred = gbdt.predict(X_test)
y_train_pred = gbdt.predict(X_train)
Y_BT_pred    =gbdt.predict(X_BT)
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\GBDT-MR.xlsx') as writer:

...
# 使用最佳模型进行预测
# 使用最佳模型进行预测
y_test_pred_scaled = model.predict(X_test_scaled)  # 确保使用带有特征名称的 DataFrame
y_train_pred_scaled = model.predict(X_train_scaled)
Y_BT_pred_scaled = model.predict(X_BT_scaled)
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\KRR-MR.xlsx') as writer:

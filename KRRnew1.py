    ...
    for individual in population_list:
        try:
            ...
            for train_index, test_index in kf.split(X):
                ...
                model = KernelRidge(
                        alpha=individual[0] / 100,
                        gamma=individual[1] / 10,
                        kernel='rbf'
                )
...
for epoch in range(epochs):
    ...
    if valid_results:
        ...
        if best_params is None or valid_results[0]['rmse'] < best_params['rmse']:
            ...
            print(
                f"Best parameters after epoch {epoch + 1}: alpha={current_best[0]}, gamma={current_best[1]}")
...
if best_params:
    ...
    final_model = KernelRidge(
        alpha=best_params['params'][0] / 100,
        gamma=best_params['params'][1]/10,
        kernel='rbf'
    )
...
# 导出到 Excel 文件
with pd.ExcelWriter(r'D:\TemporaryDirectory\KRR-DH.xlsx') as writer:
